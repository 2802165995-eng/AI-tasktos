...
让面试官通过公开 GitHub Pages 地址打开 TasteOS 网页，并让网页跨域调用阿里云函数计算 FC 的真实模型接口，无需 ICP 备案或自定义域名。
...
公开 FC URL 仍可能被他人直接调用。CORS 只能限制普通网页浏览器，不能阻止脚本或服务端请求。因此面试展示后建议禁用或轮换百炼 Key，并在后续生产化时增加鉴权、频率限制和预算告警。
